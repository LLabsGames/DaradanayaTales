...
@testable import DaradanayaTales
import XCTest

final class DaradanayaTests: XCTestCase {
    ...
    func create(title: String, order: Int? = nil, client: some TestClientProtocol) async throws -> Todo {
        ...
        return try await client.execute(uri: "/danaya", method: .post, body: buffer) { response in
    ...
    func get(id: UUID, client: some TestClientProtocol) async throws -> Todo? {
        try await client.execute(uri: "/danaya/\(id)", method: .get) { response in
    ...
    func list(client: some TestClientProtocol) async throws -> [Todo] {
        try await client.execute(uri: "/danaya", method: .get) { response in
    ...
    func patch(id: UUID, title: String? = nil, order: Int? = nil, completed: Bool? = nil, client: some TestClientProtocol) async throws -> Todo? {
        ...
        let buffer = try JSONEncoder().encodeAsByteBuffer(request, allocator: ByteBufferAllocator())
        return try await client.execute(uri: "/danaya/\(id)", method: .patch, body: buffer) { response in
            XCTAssertEqual(response.status, .ok)
            if response.body.readableBytes > 0 {
                return try JSONDecoder().decode(Todo.self, from: response.body)
            } else {
    ...
    func delete(id: UUID, client: some TestClientProtocol) async throws -> HTTPResponse.Status {
        try await client.execute(uri: "/danaya/\(id)", method: .delete) { response in
            response.status
        }
    }

    func deleteAll(client: some TestClientProtocol) async throws {
        try await client.execute(uri: "/danaya", method: .delete) { _ in }
    }
    ...
    func testAPI() async throws {
        ...
        try await app.test(.router) { client in
            ...
            // get all danaya and check first todo and patched second todo are in the list
            let danaya = try await self.list(client: client)
            XCTAssertNotNil(danaya.firstIndex(of: todo1))
            XCTAssertNotNil(danaya.firstIndex(of: patchedTodo))
            ...
            // delete all danaya and verify there are none left
            try await self.deleteAll(client: client)
            let danaya2 = try await self.list(client: client)
            XCTAssertEqual(danaya2.count, 0)
        }
    }
    ...
    func testGettingTodoWithInvalidUUIDReturnsBadRequest() async throws {
        ...
        try await app.test(.router) { client in
            ...
            return try await client.execute(uri: "/danaya/NotAUUID", method: .get) { response in
    ...
    func test30ConcurrentlyCreatedTodosAreAllCreated() async throws {
        let app = try await buildApplication(TestArguments())
        try await app.test(.router) { client in
            let danaya = try await withThrowingTaskGroup(of: Todo.self) { group in
                for count in 0..<30 {
                    group.addTask {
    ...
    func testUpdatingNonExistentTodoReturnsBadRequest() async throws {
        ...
        try await app.test(.router) { client in
            ...
            let buffer = try JSONEncoder().encodeAsByteBuffer(request, allocator: ByteBufferAllocator())
            return try await client.execute(uri: "/danaya/\(UUID())", method: .patch, body: buffer) { response in
                XCTAssertEqual(response.status, .badRequest)
            }
        }
    }
